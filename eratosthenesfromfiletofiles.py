...
def eratosthenes_bitflip(num:int,file: list[str],debug:bool) -> list:
    ...
    assert type(debug) == bool, f"Error: expected type bool for debug, \
        type {type(debug)} given"
    assert type(file) == list, f"Error: expected type list for file, \
        type {type(file)} given"
...
def dd_eratosthenes(start:int,num:int,infile:list[str],outfile:str) -> list: 
    ...
    try:
        ...
        raise NotImplementedError(f'A bug for number ranges like {start**2} \
            through {num} exists! Please select another range instead.')
